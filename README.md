...
| RULING parity — Royal / Admin | p = 0.002 / p = 0.005 | phase 3 |
| ELS Bonferroni-significant | **0** of 495 | phase 3 |
...
- **Determinism depends on your NumPy version.** Phase 3 draws every shuffle from one seeded `np.random.default_rng(42)` consumed sequentially across all four analyses. NumPy does not guarantee `Generator` stream stability across major versions, so a future NumPy could shift the ELS reference tables. It is stable across NumPy 2.x — verified end-to-end on 2.5.1. The Bonferroni-corrected null result (0/495) is not sensitive to this.
